...
def naver_name_search(text):
    """한글 종목명 → 6자리 코드 (네이버 자동완성). 실패 시 None."""
    try:
        url = "https://ac.stock.naver.com/ac"
        res = requests.get(url, params={"q": text.strip(), "target": "stock"},
                           headers={"User-Agent": "Mozilla/5.0"}, timeout=8)
        j = res.json()
        # 응답 구조 방어적 파싱: items 배열 어디서든 6자리 국내 코드 탐색
        items = j.get("items") or []
        for it in items:
            code = str(it.get("code") or it.get("cd") or "")
            if _re.match(r"^\d{6}$", code):
                return code
    except Exception as e:
        print(f"[네이버 자동완성] '{text}' 실패: {e}")
    return None
